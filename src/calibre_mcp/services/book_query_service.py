"""
Service for handling book query/read operations in the Calibre MCP application.

Split from book_service.py in Phase 3 — contains all read-only methods.
"""
...
logger = get_logger("calibremcp.services.book_query_service")
...
class BookQueryService(BaseService[Book, BookCreate, BookUpdate, BookResponse]):
    """
    Service class for handling book query/read operations.

    Provides search, filtering, detail retrieval, and formatting
    for all read-only book access patterns.
    """

    def __init__(self, db: DatabaseService):
        """
        Initialize the BookQueryService with a database instance.
    ...
    def get_all(
        ...
        **filters: Any,
        ...
        try:
            if limit < 1 or limit > 1000:
                logger.warning(
                    ...
                    extra={"service": "book_query_service", "action": "validation_error", "limit": limit},
                )
            ...
            if skip < 0:
                logger.warning(
                    ...
                    extra={"service": "book_query_service", "action": "validation_error", "skip": skip},
                )
            ...
            logger.error(
                ...
                extra={"service": "book_query_service", "action": "validation_failed", "error": str(ve)},
            )
            raise
        ...
        logger.debug(
            ...
            extra={"service": "book_query_service", "action": "session_acquired"},
        )
        try:
            ...
            if title:
                logger.debug(
                    "Filtering by title",
                    extra={"service": "book_query_service", "action": "filter_title", "title": title},
                )
                title_pattern = f"%{title}%"
                query = query.filter(Book.title.ilike(title_pattern)).distinct()
                logger.debug(
                    "Title filter applied",
                    extra={"service": "book_query_service", "action": "title_filter_applied"},
                )
            ...
            if search:
                ...
                if metadata_db_path and metadata_db_path.exists():
                    logger.debug(
                        ...
                        extra={"service": "book_query_service", "action": "find_fts"},
                    )
                    ...
                    if fts_db_path:
                        ...
                        if book_ids:
                            ...
                            logger.debug(
                                ...
                                extra={"service": "book_query_service", "action": "fts_no_results"},
                            )
                        ...
                        logger.debug(
                            ...
                            extra={"service": "book_query_service", "action": "no_fts"},
                        )
                ...
                if not fts_succeeded or book_ids is None:
                    ...
                    logger.debug(
                        ...
                        extra={"service": "book_query_service", "action": "like_filter_applied"},
                    )
            ...
            if author_id:
                ...
                if author_conditions:
                    ...
                    logger.debug(
                        ...
                        extra={"service": "book_query_service", "action": "authors_filter_applied"},
                    )
            elif author_name:
                logger.debug(
                ...
                logger.debug(
                    ...
                    extra={"service": "book_query_service", "action": "author_filter_applied"},
                )
            ...
            if series_name:
                ...
                logger.debug(
                    ...
                    extra={"service": "book_query_service", "action": "series_filter_applied"},
                )
            ...
            if tag_id:
                logger.debug(
                    "Filtering by tag_id",
                    extra={"service": "book_query_service", "action": "filter_tag_id", "tag_id": tag_id},
                )
                ...
                if tag_conditions:
                    ...
                    logger.debug(
                        ...
                        extra={"service": "book_query_service", "action": "tags_filter_applied"},
                    )
            elif tag_name:
                logger.debug(
                ...
                logger.debug(
                    ...
                    extra={"service": "book_query_service", "action": "tag_filter_applied"},
                )
            ...
            if exclude_tags_list:
                ...
                if exclude_tag_conditions:
                    ...
                    logger.debug(
                        ...
                        extra={"service": "book_query_service", "action": "tags_excluded"},
                    )
            ...
            if exclude_authors_list:
                ...
                if exclude_author_conditions:
                    ...
                    logger.debug(
                        ...
                        extra={"service": "book_query_service", "action": "authors_excluded"},
                    )
            ...
            if exclude_series_list:
                ...
                if exclude_series_conditions:
                    ...
                    logger.debug(
                        ...
                        extra={"service": "book_query_service", "action": "series_excluded"},
                    )
            ...
            if comment:
                ...
                logger.debug(
                    ...
                    extra={"service": "book_query_service", "action": "comment_filter_applied"},
                )
            ...
            if has_cover is not None:
                ...
                logger.debug(
                    ...
                    extra={"service": "book_query_service", "action": "cover_filter_applied"},
                )
            ...
            if pubdate_start or pubdate_end:
                if pubdate_start:
                ...
                if pubdate_end:
                    ...
                    if dt:
                        ...
                        # If the date has no time component, extend to end of day
                        if dt.hour == 0 and dt.minute == 0 and dt.second == 0:
                            dt = dt + timedelta(days=1)
                        query = query.filter(Book.pubdate <= dt)

            if added_after or added_before:
                if added_after:
            ...
            if min_size is not None or max_size is not None:
                # Size is stored in the data table (uncompressed_size)
                # We need to join data and filter
                q_ids = session.query(Book.id).join(Book.data)
                if min_size is not None:
                    q_ids = q_ids.filter(Book.data.property.mapper.class_.uncompressed_size >= min_size)
                if max_size is not None:
                    q_ids = q_ids.filter(Book.data.property.mapper.class_.uncompressed_size <= max_size)
                q_ids = q_ids.distinct().subquery()
                query = query.filter(Book.id.in_(session.query(q_ids.c.id)))

            # Apply formats filter
            formats_list = filters.pop("formats", None)
            if formats_list:
                q_ids = (
                    session.query(Book.id)
                    .join(Book.data)
                    .filter(
                        Book.data.property.mapper.class_.format.in_([f.upper() for f in formats_list])
                    )
                    .distinct()
                    .subquery()
                )
            ...
            # Apply has_empty_comments filter
            has_empty_comments = filters.pop("has_empty_comments", None)
            if has_empty_comments is not None:
                if has_empty_comments:
                    # Find books with no comments or empty comments
                    c_subq = (
                        session.query(Comment.book_id)
                        .filter(Comment.text.isnot(None), Comment.text != "")
                        .distinct()
                        .subquery()
                    )
                    query = query.filter(~Book.id.in_(session.query(c_subq.c.book_id)))
                else:
                    # Find books with non-empty comments
                    query = query.join(Comment, Comment.book_id == Book.id).filter(
                        Comment.text.isnot(None), Comment.text != ""
                    )
                    query = query.distinct()

            # Apply publisher filters
            publisher_name = filters.pop("publisher", None)
            publishers_list = filters.pop("publishers", None)
            has_publisher = filters.pop("has_publisher", None)
            if publisher_name:
                pub_pattern = f"%{publisher_name}%"
                from ..db.models import Publisher
                pub_ids = (
                    session.query(Book.id)
                    .join(Book.publishers)
                    .filter(Publisher.name.ilike(pub_pattern))
                    .distinct()
                    .subquery()
                )
                query = query.filter(Book.id.in_(session.query(pub_ids.c.id)))
            if publishers_list:
                from ..db.models import Publisher
                pub_conditions = [Publisher.name.ilike(f"%{p}%") for p in publishers_list]
                pub_ids = (
                    session.query(Book.id)
                    .join(Book.publishers)
                    .filter(or_(*pub_conditions))
                    .distinct()
                    .subquery()
                )
                query = query.filter(Book.id.in_(session.query(pub_ids.c.id)))
            ...
            if sort_column == "title":
                ...
                order_col = Book.author_sort
            elif sort_column == "series":
                from sqlalchemy import func as sa_func
                # Join series and order by series name + series_index
                query = query.outerjoin(Book.series)
                order_col = sa_func.coalesce(Series.name, "")
            elif sort_column == "rating":
                from sqlalchemy import func as sa_func
                query = query.outerjoin(Book.ratings)
                order_col = sa_func.coalesce(Rating.rating, 0)
            elif sort_column == "timestamp":
                order_col = Book.timestamp
            elif sort_column == "pubdate":
                order_col = Book.pubdate
            else:
                order_col = Book.sort
            ...
            # Add stable secondary sort to ensure consistent pagination
            # Use ID as tiebreaker when primary sort is not unique
            if sort_column not in ("timestamp", "pubdate", "id"):
                query = query.order_by(Book.id.desc())

            # Get total count
            logger.debug(
                "Counting total results",
                extra={"service": "book_query_service", "action": "count_start"},
            )
            total = query.count()
            logger.info(
                "Total results count",
            ...
            for book in books:
                try:
                    ...
                    # Attach FTS snippet if available
                    if fts_snippets and book.id in fts_snippets:
                        book_dict["snippet"] = fts_snippets[book.id]
                    result_books.append(book_dict)
                except Exception as e:
                    logger.error(
                        "Error converting book to response",
                        extra={
                            "service": "book_query_service",
...
book_query_service = BookQueryService(DatabaseService())
